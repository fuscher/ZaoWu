...
function downloadGit() {
  (window as any).open('https://git-scm.com/', '_blank')
}
</script>
  ...
  <div class="dialog-overlay" @click.self="emit('close')">
    <div class="dialog">
      <div class="dialog-body">
        ...
        <div class="dialog-title">{{ t('git.gitNotInstalled') }}</div>
        <div class="dialog-desc">{{ t('git.gitNotInstalledDesc') }}</div>
      </div>
      <div class="dialog-footer">
        <button class="dialog-btn secondary" @click="emit('close')">{{ t('git.close') }}</button>
        <button class="dialog-btn primary" @click="downloadGit">{{ t('git.downloadGit') }}</button>
      </div>
    </div>
  </div>
</template>
