...
| 能力 | 说明 |
| --- | --- |
| AI 对话与 Agent | 兼容 OpenAI 接口的 Provider 接入，流式输出、多轮对话、模型切换与生成参数调节；Agent 模式支持工具调用、沙箱执行与人工审批；支持 @ 文件/项目引用与项目沙箱，引用按 token 截断并纳入上下文压缩预算 |
...
| 更新与分发 | 三源版本检测、流式下载、Go 启动器实现版本无缝切换与失败回滚 |
