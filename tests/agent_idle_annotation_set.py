"""空转识别标注集（设计文档 §8.2 / D1）— 54 条人工标注的 text-only 轮次样本。

29 正例 = 应判 idle/constrained（说而不做）：
- 承诺类 20 条（"我先读取…"、"让我检查…" 等有承诺无执行的文本）
- 建议陷阱 5 条（"建议你手动删除 X"——建议非承诺、无结论信号 → 应 idle）
- 承诺变体 3 条（"我先定位…"——承诺性定位短语、无结论信号 → 应 idle，防 D1 漏测）
- 危险变体 1 条（"我先定位到问题所在"，2026-08-07 补入——承诺未兑现但含"到"，
  若结论词保留"定位到"会被误判 success，防 D1 漏测）
